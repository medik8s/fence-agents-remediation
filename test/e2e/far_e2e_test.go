	...
	runFARTests := func(remediationStrategy v1alpha1.RemediationStrategyType, testAction string, skipCondition func() bool) {
		...
		JustBeforeEach(func() {
			...
			if testAction == "off" {
				...
				ctx = context.WithValue(ctx, e2eUtils.ClientSetContextKey, clientSet)
				ctx = context.WithValue(ctx, e2eUtils.ControllerNSContextKey, operatorNsName)
				ctx = context.WithValue(ctx, e2eUtils.LoggerContextKey, log)

				DeferCleanup(e2eUtils.PowerOnNode, ctx, far, selectedNode.Name)
			}
		})
