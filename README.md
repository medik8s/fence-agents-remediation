...
FAR adds a unique taint, `medik8s.io/fence-agents-remediation`, with [NoSchedule effect](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/#taint-based-evictions), so any new pods can't be scheduled on the unhealthy node as long as the taint remains (the taint is removed on far CR deletion).
